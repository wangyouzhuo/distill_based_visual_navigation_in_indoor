...
class ACNet(object):
        ...
        with tf.device(device):
            ...
            if scope == 'Global_Net':  # get global network
                with tf.variable_scope(scope):
                    ...
                    self.special_a_params_dict,self.special_c_params_dict =\
                ...
                with tf.variable_scope(scope):
                    ...
                    #self.a_onehot = tf.placeholder(tf.int32, [None, self.dim_s], 'Action_one_hot')
                    self.T = tf.placeholder(tf.float32,None,'Temperature')
    ...
    #     with tf.variable_scope(scope):
    #         q_input = tf.concat([self.s,self.a_onehot],axis=1)
    ...
    def _prepare_global_loss(self,scope):
        with tf.name_scope(scope+'global_loss'):
            ...
            self.reg_td = tf.subtract(tf.stop_gradient(self.special_v), self.global_v, name='reg_TD_error')
            with tf.name_scope('global_c_loss'):
                ...
                #self.global_c_loss = BETA_REG_VALUE*reg_c_loss + glo_c_loss
                self.global_c_loss = glo_c_loss
    ...
    def _prepare_special_grads(self,scope):
        with tf.name_scope(scope+'special_grads'):
            with tf.name_scope('special_net_grad'):
                ...
                self.spcial_c_grads = [tf.clip_by_norm(item, 40) for item in
                                tf.gradients(self.special_c_loss, self.special_c_params)]

                # self.spcial_q_grads = [tf.clip_by_norm(item, 40) for item in
    ...
    def _prepare_update_op(self,scope):
        ...
        with tf.name_scope(scope+'_special_update'):
            ...
            for key in TARGET_ID_LIST:
                ...
                kv_c = {key: self.OPT_REG_C.apply_gradients(list(zip(self.spcial_c_grads , self.global_AC.special_c_params_dict[key])))}
    ...
    def update_special(self, feed_dict,target_id=None):  # run by a local
    ...
    def pull_global(self):
        ...
        # glo_a_params, local_a_params = self.session.run([self.global_AC.global_a_params, self.global_a_params])
        # print("global_a_params:%6s || local_a_params：%6s" % (
        # round(np.sum(glo_a_params[-2]), 3), round(np.sum(local_a_params[-2]), 3)))
